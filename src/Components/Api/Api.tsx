...
export const sendToSpecialist = async (
  { amount, accountNumber }: any,
  id: any
) => {
  return await axios
    .patch(
      `${lifeUrl}/sendtospecialist/${id}`,
...
export const sendToHospital = async (id: any) => {
  return await axios
    .patch(`.${lifeUrl}/sendtohospital/${id}/${id}`)
    .then((res) => res.data);
};

export const sendToOtherWallet = async (id: any) => {
  return await axios.patch(`${lifeUrl}/transfer/${id}`).then((res) => res.data);
  // .patch(`http://localhost:2001/api/transfer/${id}`).then((res) => res.data)
};

export const fundFromBank = async (
  { amount, number, cvv, expiry_month, expiry_year }: iData,
  id: string
) => {
  return await axios
    .post(
      `${lifeUrl}/fundWallet/${id}`,
...
export const bookAppointment = async (id: any) => {
  ...
  return await axios
    .post(`${lifeUrl}/bookappointment/${id}/${id}`)
    .then((res) => res.data);
